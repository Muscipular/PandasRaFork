...
uint64 MobItemFixedRatioDB::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (!item_db.exists(nameid)) {
		this->invalidWarning(node, "Unknown item ID %u in MobItem Fixed Ratio Database.\n", nameid);
		return 0;
	}
...
uint32 mobdrop_fixed_droprate_adjust(uint32 nameid, uint32 mobid, uint32 rate) {
	...
	if (!data->monsters.empty()) {
		auto iter = std::find(data->monsters.begin(), data->monsters.end(), mobid);

		if (iter == data->monsters.end()) {
			return rate;
		}
	}
